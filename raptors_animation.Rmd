...
scoring_df <- scoring_plays %>%
  mutate(scoring_home_player = str_extract(home_action, "[A-Z]\\. [A-Za-z-]+"),
         ...
         scoring_player = str_remove(paste0(scoring_home_player, scoring_away_player), "(NA)"),
...
if (home_team == winner){
  final_game_sequence_df <- game_sequence_df %>%  
    mutate(winning_score = sprintf("%02d", as.numeric(str_extract(running_score, "^[0-9]+") )),
           ...
           running_score = str_glue("{winning_score}-{away_score}"))
} else {
  final_game_sequence_df <- game_sequence_df %>%  
    mutate(winning_score = sprintf("%02d", as.numeric(str_extract(running_score, "[0-9]+$"))),
           away_score = sprintf("%02d", as.numeric(str_extract(running_score, "^[0-9]+") )),
...
```{r opponent_colour}
# opponent colour from https://teamcolorcodes.com/nba-team-color-codes/
