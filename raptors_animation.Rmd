...
```{r page_link}
page_url <- "https://www.basketball-reference.com/boxscores/pbp/202003090UTA.html"
...
opponent_path <- image_paths %>%
  filter(str_detect(path, str_extract(opponent, "[a-zA-Z]+$"))) %>%
  .$path
...
base_plot <- animation_df %>%
  ggplot(aes(x = scoring_player, 
   ...
   annotation_custom(raptors_alt_logo, 
                     ymin = 21, ymax= 27,
                     xmin = length(x_axis_players) * 0.225, 
                     xmax = length(x_axis_players) * 0.425) +
...
#geom_text(data = animation_df %>% filter(points == 1), aes(label = "—"), size = 8, colour = "white", alpha = 0.7, hjust = -0.05) +
 ...
 geom_text(data = final_frame_scores, 
  ...
  scale_y_continuous("Baskets Scored",
                     breaks = seq(0, 25, by = 5)) + 
  labs(title = str_glue("<b style='color:{team_colour};font-size:32px'>TORONTO RAPTORS</b><br><b style='font-size:25px'>\\>DROPPIN' BASKETS</b>"),
...
animate(plot = animated_plot,
        ...
        renderer = av_renderer(str_glue(
          "animations/{team_of_interest}_{opponent}-{game_date}.mp4"))
        )
