...
```{r page_link}
page_url <- "https://www.basketball-reference.com/boxscores/pbp/201912280BOS.html"
```
...
base_plot <- animation_df %>%
  ...
  annotation_custom(winner_logo, 
                    xmin = -Inf, xmax = Inf, 
                    ymin = -Inf, ymax = Inf) +
  # geom_point(data = animation_df %>% filter(points == 3), shape = 21, size = 20, fill = winner_colour, colour = "black") +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        plot.title = element_markdown(size = 45, face = "bold", colour = "#000000",
                                      lineheight = 1, vjust = 0, hjust = 0.5)) +
  expand_limits(y = 15)
```

```{r, dpi = 300}
animated_plot <- base_plot +
  transition_states(running_score, transition_length = 0.8, state_length = 0.3, wrap = FALSE) +
