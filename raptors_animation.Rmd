...
team_animation_df <- animation_df %>%
  filter(!is.na(team)) %>%
  mutate(team = factor(team, levels = c("Toronto Raptors", "Los Angeles Clippers")))
...
```{r}
anim_save(str_glue("animations/raptors_clippers-{str_extract(page_url, '[0-9]+')}.gif"), anim, fps = 25, duration = nrow(animation_df)/3.5, type = "cairo-png", width = 1024, height = 512, renderer = gifski_renderer(loop = FALSE))
