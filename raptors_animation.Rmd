...
```{r page_link}
page_url <- "https://www.basketball-reference.com/boxscores/pbp/201912290TOR.html"
```
...
end_of_game <- data.frame(running_score = tail(scoring_df$running_score, 1), time = "0:00")
...
```{r}
# # see the amount of difference between characters in title and determine which side needs to be balanced out
# if (nchar(winner) < nchar(loser)){
#   white_space_needed <- c(rep("a", times = nchar(loser) - nchar(winner))) %>%
#     str_flatten()
#   
#   centred_title <- paste0("<b style='color:#ffffff'>", white_space_needed, "</b>", "<b style='color:", winner_colour, "'>", winner, "</b> {closest_state} <b style='color:", loser_colour, "'>", loser, "</b>")
#   
# } else {
#   white_space_needed <- c(rep("a", times = nchar(winner) - nchar(loser))) %>%
#     str_flatten()
#   
#   centred_title <- paste0("<b style='color:", winner_colour, "'>", winner, "</b> {closest_state} <b style='color:", loser_colour, "'>", loser, "</b>", "<b style='color:#ffffff>", white_space_needed, "</b>")
# }
```
...
if (nchar(winner) < nchar(loser)){
  ...
  centred_title <- paste0("<b style='color:#ffffff'>", white_space_needed, "</b>", "<b style='color:", winner_colour, "'>", winner, "</b>", " def. ", "<b style='color:", loser_colour, "'>", loser, "</b>")
  
} else {
  white_space_needed <- c(rep("a", times = nchar(winner) - nchar(loser))) %>%
    str_flatten()
  
  centred_title <- paste0("<b style='color:", winner_colour, "'>", winner, "</b>", " def. ", "<b style='color:", loser_colour, "'>", loser, "</b>", "<b style='color:#ffffff>", white_space_needed, "</b>")
}
```

```{r, warning = FALSE, dev = "CairoPNG", fig.width = 10.24, fig.height= 5.12, dpi = 300}
base_plot <- animation_df %>%
  ggplot(aes(x = scoring_player, y = player_baskets, group = action_order)) +
  annotation_custom(winner_logo, 
                    xmin = -Inf, xmax = Inf, 
                    ymin = -Inf, ymax = Inf) +
  geom_point(data = animation_df %>% filter(points == 3), shape = 21, size = 18, fill = winner_colour, colour = "black") +
  geom_image(aes(image = str_glue("{here::here()}/emoji/basketball.png")), size = 0.06) +
  scale_x_discrete("",
...
animated_plot <- base_plot +
  transition_states(running_score, transition_length = 0.8, state_length = 0.1, wrap = FALSE) +
